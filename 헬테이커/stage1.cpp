...
HRESULT stage1::init()
{
	...
	if (clear == false || reset == false);
	{
		...
		wall[0].rc = RectMakeCenter(WINSIZEX / 2 + 150, WINSIZEY / 2 - 375, 200, 100);
		wall[1].rc = RectMakeCenter(WINSIZEX / 2 - 100, WINSIZEY / 2 - 275, 300, 100);
		wall[2].rc = RectMakeCenter(WINSIZEX / 2 - 300, WINSIZEY / 2 - 125, 100, 200);
		wall[3].rc = RectMakeCenter(WINSIZEX / 2 - 400, WINSIZEY / 2 + 125, 100, 300);
		wall[4].rc = RectMakeCenter(WINSIZEX / 2, WINSIZEY / 2 + 325, 700, 100);
		wall[5].rc = RectMakeCenter(WINSIZEX / 2 + 400, WINSIZEY / 2 + 175, 100, 200);
		wall[6].rc = RectMakeCenter(WINSIZEX / 2 + 300, WINSIZEY / 2 + 125, 100, 100);
		wall[7].rc = RectMakeCenter(WINSIZEX / 2 + 50, WINSIZEY / 2 + 25, 400, 100);
		wall[8].rc = RectMakeCenter(WINSIZEX / 2 + 200, WINSIZEY / 2 - 75, 100, 100);
		wall[9].rc = RectMakeCenter(WINSIZEX / 2 + 300, WINSIZEY / 2 - 225, 100, 200);

		player.img = IMAGEMANAGER->findImage("오른쪽 기본");
		heroine.img = IMAGEMANAGER->findImage("히로인1");
		...
		player.x = WINSIZEX / 2 + 200;
		player.y = WINSIZEY / 2 - 275;

		heroine.x = WINSIZEX / 2 + 300;
		heroine.y = WINSIZEY / 2 + 225;
		heroine.rc = RectMakeCenter(heroine.x, heroine.y, 80, 80);

		enemy[0].x = WINSIZEX / 2 - 100;
		enemy[0].y = WINSIZEY / 2 - 175;

		enemy[1].x = WINSIZEX / 2 - 200;
		enemy[1].y = WINSIZEY / 2 - 75;

		enemy[2].x = WINSIZEX / 2;
		enemy[2].y = WINSIZEY / 2 - 75;

		obs[0].x = WINSIZEX / 2 - 200;
		obs[0].y = WINSIZEY / 2 + 125;

		obs[1].x = WINSIZEX / 2 - 200;
		obs[1].y = WINSIZEY / 2 + 225;

		obs[2].x = WINSIZEX / 2;
		obs[2].y = WINSIZEY / 2 + 225;

		obs[3].x = WINSIZEX / 2 + 100;
		obs[3].y = WINSIZEY / 2 + 125;
