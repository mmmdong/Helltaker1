...
HRESULT stage3::init()
{
	...
	if (clear == false || reset == false)
	{
		...
		heroine.img = IMAGEMANAGER->findImage("히로인3");
		_hit = new effect;
		...
		wall[0].rc = RectMakeCenter(WINSIZEX / 2 + 150 , WINSIZEY / 2 - 425, 300, 100);
		wall[1].rc = RectMakeCenter(WINSIZEX / 2 - 150, WINSIZEY / 2 - 275, 300, 200);
		wall[2].rc = RectMakeCenter(WINSIZEX / 2 - 350, WINSIZEY / 2 + 75, 100, 500);
		wall[3].rc = RectMakeCenter(WINSIZEX / 2 - 100, WINSIZEY / 2 + 375, 400, 100);
		wall[4].rc = RectMakeCenter(WINSIZEX / 2 + 150, WINSIZEY / 2 + 225, 100, 200);
		wall[5].rc = RectMakeCenter(WINSIZEX / 2 + 250, WINSIZEY / 2 + 175, 100, 100);
		wall[6].rc = RectMakeCenter(WINSIZEX / 2 + 350, WINSIZEY / 2 - 75, 100, 400);
		wall[7].rc = RectMakeCenter(WINSIZEX / 2 + 300, WINSIZEY / 2 - 325, 200, 100);
		wall[8].rc = RectMakeCenter(WINSIZEX / 2 - 50, WINSIZEY / 2 - 125, 100, 100);
		wall[9].rc = RectMakeCenter(WINSIZEX / 2 - 150, WINSIZEY / 2 + 125, 100, 200);
		wall[10].rc = RectMakeCenter(WINSIZEX / 2 - 50, WINSIZEY / 2 + 175, 100, 100);
		...
		player.x = WINSIZEX / 2 + 250;
		player.y = WINSIZEY / 2 + 75;

		key.x = WINSIZEX / 2 - 150;
		key.y = WINSIZEY / 2 - 125;

		box.x = WINSIZEX / 2 + 150;
		box.y = WINSIZEY / 2 - 225;

		heroine.x = WINSIZEX / 2 + 50;
		heroine.y = WINSIZEY / 2 - 325;
		heroine.rc = RectMakeCenter(heroine.x, heroine.y, 80, 80);


		enemy[0].x = WINSIZEX / 2 - 250;
		enemy[0].y = WINSIZEY / 2 - 25;

		enemy[1].x = WINSIZEX / 2 + 50;
		enemy[1].y = WINSIZEY / 2 - 25;

		enemy[2].x = WINSIZEX / 2 - 50;
		enemy[2].y = WINSIZEY / 2 + 75;

		obs[0].x = WINSIZEX / 2 + 50;
		obs[0].y = WINSIZEY / 2 - 125;

		obs[1].x = WINSIZEX / 2 + 150;
		obs[1].y = WINSIZEY / 2 - 125;

		obs[2].x = WINSIZEX / 2 + 250;
		obs[2].y = WINSIZEY / 2 - 125;

		obs[3].x = WINSIZEX / 2 + 150;
		obs[3].y = WINSIZEY / 2 - 25;

		obs[4].x = WINSIZEX / 2 - 150;
...
void stage3::render()
{
	if (clear == false || reset == false)
	{
		...
		IMAGEMANAGER->findImage("히로인3");
		heroine.img->aniRender(getMemDC(), heroine.rc.left - 15, heroine.rc.top - 10, heroine.ani);
...
void stage3::collision()
{
	...
	for (int i = 0; i < WALLMAX; i++)
	{
		if (IntersectRect(&temp, &inter[i].rc, &box.rc))
		{
			if (box.leftmove)
			{
